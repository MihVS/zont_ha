...
Для котлового адаптера, если ZONT передаёт соответствующие флаги, добавляются бинарные
сенсоры `ch`, `dhw` и `fl`: отопление включено, ГВС включено и горелка работает.
Данные берутся из `status` адаптера. Положение трёхходового крана интеграция не определяет.
