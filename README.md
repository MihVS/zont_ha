...
Для адаптера цифровой шины добавляются бинарные сенсоры состояния `ch`, `dhw` и `fl`:
отопление включено, ГВС включено и горелка работает. Данные берутся из `status` адаптера.
Это состояние, переданное котлом; положение трёхходового крана интеграция не определяет.
...
Webhook нужен для мгновенного изменения состояния устройства в НА. Например если вы
измените заданную температуру контура в приложении ZONT, то сервер ZONT оповестит
ваш НА об изменении параметра и НА отобразит актуальные данные. Так же можно настроить 
уведомления об аварии котла, сработки охранной зоны и тд.

Настраивается в меню Аккаунта (В браузерной версии слева вверху жмём на аккаунт) -
Интеграции - Webhook. Если данного пункта меню
